...
var SiteMgr = {
    ...
    loadConfigItems: function() {
        ...
        $.getJSON(dictPageUrl + 'getDepth1Items', {
            ...
            for (var i = 0; i < SiteMgr.categories.length; i++) {
                ...
                var gridId = id + "-grid";
                var content = "<div id=\"" + id + "\" style=\"padding: 2px;\">" + "<div style=\"height:" + height + "px\">" + "<div class=\"easyui-datagrid\" id=\"" + gridId + "\"></div>" + "</div>" + "</div>";
                $('#param-tabs').tabs('add', {
                    title: title,
                    content: content
                });
    ...
    toolbar: {
        ...
        querySchedule: function() {
            var node = $('#site-tree').tree('getSelected');
            var map = SiteMgr.toolbar.getSelectedParamUidList();
            if (node && map.paramUidList.length > 0) {} else {
                $.messager.alert('警告', '请选中一个站点或设备,并确定是否选中参数!', 'info');
            }
        },
        cancel: function() {
