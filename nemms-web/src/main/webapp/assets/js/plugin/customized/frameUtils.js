...
**  比较时间  小于被比较时间 false
**  比较时间  大于被比较时间 true
**	如 输入   :   2009-09-09,2009-09-10  return true
**  时间格式    yyyy-MM-dd
**/
XFrameUtils.compareDate = function(inputDate,compareDate){
	...
	if(inputDateArray[0]==undefined||compareDateArray[0]==undefined
		||inputDateArray[1]==undefined||compareDateArray[1]==undefined
		||inputDateArray[2]==undefined||compareDateArray[2]==undefined){
		return false;
	}
...
/**
 * 字符串是否为空,长度为0的字符串、全是空格组成的字符串都认为是空。
...
/**
 * 字符串是否不为空,长度为0的字符串、全是空格组成的字符串都认为是空。
...
/**
 * 验证英文，可包含字母、数字、下划线、连接线
 * Return: true 是英文、false不是英文
 */
...
/**
 * 验证URL地址，可包含字母、数字、下划线、连接线、点、斜线、冒号
 * Return: true 是英文、false不是英文
 */
...
/**
 * 搜索条点击响应函数，可以折叠或打开搜索条。
 * Param objSearch 搜索条的DOM对象。
 */
XFrameUtils.onclickSearchBar = function(objSearch)
{
	...
	if($("#"+divId).css("display") == 'none')
	{
		...
		$("#"+imgId).attr("src",XFrame.getContextPath() + "/images/closeField.gif"); 
	}else{
		$("#"+divId).hide();
		$("#"+imgId).attr("src",XFrame.getContextPath() + "/images/openField.gif"); 
	}
}
