...
// loadScript
;(function(window, undefined) {
	var scriptStatus = {}, scriptQueues = [];
	var STATUS_READY = 0, STATUS_LOADING = 1, STATUS_FINISH = 2, STATUS_FAILE = 3;

	var emitFinish = function(src) {
		for(var i = 0; i < scriptQueues.length; i++) {
			var queue = scriptQueues[i];
			...
			if(queue.length === 1 && typeof queue[0] === 'function') {
				var callback = queue[0];
				scriptQueues.splice(i, 1);
				callback();
				i--;
			}
		}
	...
	window.loadScript = function(scriptName, callback) {
		if(typeof scriptName === 'string') {
			scriptName = [scriptName];
		}

		if(Object.prototype.toString.call(scriptName) === '[object Array]') {
			if(callback) scriptQueues.push(scriptName.concat(callback));
			
			for(var i = 0, len = scriptName.length; i < len; i++) {
				var src = scriptName[i];
				if(scriptStatus[src] === undefined) {
					scriptStatus[src] = STATUS_READY;
					var body = document.body;
					var script = document.createElement('script');
					script.src = script.dataSrc = src;
					script.defer = true;

					script.onload = function() {
						scriptStatus[this.dataSrc] = STATUS_FINISH;
						emitFinish(this.dataSrc);
					}

					script.onerror = function() {
						scriptStatus[this.dataSrc] = STATUS_FAILE;
						throw('脚本：' + this.dataSrc + ' 加载出错');
					}

					body.appendChild(script);
					scriptStatus[src] = STATUS_LOADING;
				} else if(scriptStatus[src] === STATUS_FINISH) {
					emitFinish(src);
				}
			}
		}
...
if($.navAsAjax)
{
    ...
    $(document).on('click', 'nav a[href!="#"]', function(e) {
		...
		if (!$this.parent().hasClass("active") && !$this.attr('target') || $this.hasClass('top-level-menu') || hash !== href) {
...
function loadURL(url, container) {
	...
	$.ajax({
		...
		beforeSend : function() {
			...
			$(document).trigger('page_destroy').off('page_destroy');
		},
		/*complete: function(){
...
function drawBreadCrumb() {
	...
	nav_elems.each(function() {
		...
		var nav = $(this).clone().children(".badge").remove().end(),
			href = nav.attr('href'),
			text = nav.text();

		if (!--count) {
			document.title = nav_elems.eq(count - 1).attr('title') || $.bread_crumb.find("li:last-child").text();
			$.bread_crumb.append($('<li>' + text + '</li>'));
		} else {
			$.bread_crumb.append($('<li></li>').html('<nav><a href="' + href + '">' + text + '</a></nav>'));
		}
	});
...
if(typeof template === 'function') {
	template.helper('numberFormat', function(num, format) {
		return $.number(num);
	})
	template.helper('number2percent', function(num, format) {
...
;(function($) {
	var html = [
		'<div class="loading-dialog">',
			'<span class="ver-middle-helper"></span>',
			'<div class="loading-body">',
				'<div class="progress-tips">正在加载，请稍等...</div>',
				'<div class="progress">',
					'<div class="progress progress-striped active">',
						'<div class="progress-bar bg-color-blue-cus" role="progressbar" style="width: 100%"></div>',
					'</div>',
				'</div>',
				'<span class="fa fa-times loading-close"></span>',
			'</div>',
		'</div>'].join('');

	$.loading = function(title) {
		var loading = $(html).appendTo($('body'));
		if(title) {
			loading.find('.progress-tips').text(title);
		}
		return {
			show: function() {
				loading.show();
	...
	$('body').on( 'click', '.loading-close',function() {
		$(this).parents('.loading-dialog').remove();
	})
})(jQuery);

(function($) {
	var getTmpl = function(type, msg) {
		type = type === 'error' ? ' error-message' : '';
		return '<div class="global-message' + type + '" id="globalMessageInstance">\
	...
	$.message = {
		showError: function(msg) {
			$('#globalMessageInstance').remove();
			$('body').append(getTmpl('error', msg));
			setTimeout(function() {
				$('#globalMessageInstance').fadeOut(function() {
					$('#globalMessageInstance').remove();
				});
			}, 4000);
		},
		showSuccess: function(msg) {
			$('#globalMessageInstance').remove();
			$('body').append(getTmpl('success', msg));
			setTimeout(function() {
				$('#globalMessageInstance').fadeOut(function() {
					$('#globalMessageInstance').remove();
				});
